...
import { FaHome, FaUsers, FaClipboardList, FaChartBar, FaSignOutAlt, FaBars, FaTimes, FaGraduationCap, FaSyncAlt, FaSpinner, FaCheck } from "react-icons/fa";
...
function AdminDashboard() {
  ...
  const [stats, setStats] = useState({
    ...
    completedApplications: 0, // Count of completed internships (status: "Completed")
  });
  ...
  // Poll for stats every 30 seconds to auto-update completed count
  useEffect(() => {
  ...
  const fetchStats = async () => {
    try {
      ...
      setStats({
        ...
        completedApplications: data.completedApplications || 0, // Backend: count({ status: "Completed" })
      });
    } catch (err) {
  ...
  // Manual refresh stats (e.g., after completing applications)
  ...
  const StatsCard = ({ title, count, color, icon: customIcon, delay = 0, isLoading = false }) => {
    const icons = {
      ...
      totalInternships: <FaHome className="text-4xl" />, // Consistent icon
      totalInternshipsApplied: <FaClipboardList className="text-4xl" />,
      ...
      completedApplications: <FaGraduationCap className="text-4xl" />, // For completed
    };
        ...
        return (
          <div className="p-4 md:p-6 animate-slide-down">
            <div className="flex items-center justify-between mb-6">
              <h1 className="text-2xl md:text-3xl lg:text-4xl font-bold text-gray-800 bg-gradient-to-r from-blue-600 to-indigo-700 bg-clip-text text-transparent animate-[fade-in-up_0.8s_ease-out] animate-float">
                Welcome, Admin 👋
              </h1>
              <button
                onClick={handleRefreshStats}
                disabled={isLoadingStats}
                className="flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-all duration-300 transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed animate-pulse-glow"
              >
            ...
            <div className="stats-section grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6 mb-8">
              {isLoadingStats ? (
                <>
                  <StatsCard title="Total Students" count={0} color="blue" isLoading={true} delay={0} />
                  <StatsCard title="Total Internships" count={0} color="green" isLoading={true} delay={1} />
                  <StatsCard title="Total Internships Applied" count={0} color="indigo" isLoading={true} delay={2} />
                  <StatsCard title="Pending Applications" count={0} color="yellow" isLoading={true} delay={3} />
                  <StatsCard title="Accepted Applications" count={0} color="green" isLoading={true} delay={4} />
                  <StatsCard title="Rejected Applications" count={0} color="red" isLoading={true} delay={5} />
                  <StatsCard title="Completed Applications" count={0} color="purple" isLoading={true} delay={6} />
                </>
              ) : (
                <>
                  <StatsCard title="Total Students" count={stats.totalStudents} color="blue" delay={0} />
                  <StatsCard title="Total Internships" count={stats.totalInternships} color="green" delay={1} />
                  <StatsCard title="Total Internships Applied" count={stats.totalInternshipsApplied} color="indigo" delay={2} />
                  <StatsCard title="Pending Applications" count={stats.pendingApplications} color="yellow" delay={3} />
                  <StatsCard title="Accepted Applications" count={stats.acceptedApplications} color="green" delay={4} />
                  <StatsCard title="Rejected Applications" count={stats.rejectedApplications} color="red" delay={5} />
                  <StatsCard title="Completed Applications" count={stats.completedApplications} color="purple" delay={6} icon={<FaGraduationCap className="text-4xl" />} />
                </>
              )}
            </div>
            {/* Summary */}
            <div className="bg-white p-4 rounded-xl shadow-md border border-gray-200 animate-[fade-in-up_0.8s_ease-out]">
              ...
              <p className="text-gray-600">
                Total Completed Internships: <span className="font-bold text-purple-600">{stats.completedApplications}</span> students have successfully finished their internships!
              </p>
            </div>
          </div>
        );
